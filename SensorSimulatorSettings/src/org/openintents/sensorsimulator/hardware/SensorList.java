...
public class SensorList {
	...
	private ArrayList<Sensor> currentSensors = new ArrayList<Sensor>();
	...
	protected boolean checkList(int type) {
		Iterator<Sensor> iter = currentSensors.iterator();
		while (iter.hasNext()) {
	 ...
	 */
	protected Sensor addSensor(int type) {
	...
	protected void removeSensor(int type) {
		if (currentSensors.size() == 1) {
			currentSensors = new ArrayList<Sensor>();
		} else {
			for (int i = 0; i < currentSensors.size(); i++) {
				if (currentSensors.get(i).getType() == type) {
	 ...
	 * 
	 * @return currentSensors, ArrayList<Sensor> of currently enabled sensors.
	 */
	protected ArrayList<Sensor> getList() {
